...
export class AppController {
  ...
  async redirect(@Query('url') url: string) {
    const allowedDomains = ['example.com', 'another-example.com']; // Define allowed domains
    try {
  ...
  getSecrets(): Record<string, string> {
    // Secrets should not be hardcoded. They should be retrieved from a secure source.
    const secrets = {
